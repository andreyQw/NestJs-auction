...
export class LotsService {
  ...
  async addLotJobs(lot: Lot): Promise<void> {
    ...
    const startTime = moment.utc(lot.lotStartTime).diff(currentTime, 'milliseconds');
    const closeTime = moment.utc(lot.lotEndTime).diff(currentTime, 'milliseconds');

    await this.lotsQueue.add('startLot', {lotId: lot.id}, { delay: startTime, jobId: `startLot${lot.id}`});
    await this.lotsQueue.add('closeLot', {lotId: lot.id}, { delay: closeTime, jobId: `closeLot${lot.id}`});
  }
  ...
  async removeLotJobs(lot: Lot): Promise<void> {
    ...
    if (closeJob) { await closeJob.remove() }
  }
}
